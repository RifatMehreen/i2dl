...
def affine_forward(x, w, b):
    ...
    # Make sure reshaping is done correctly
    assert x_2d.shape == (N, D)
    out = np.dot(x_2d, w) + b
    cache = (x, w, b)
    return out, cache
...
def relu_forward(x):
    ...
    """
    out = None
    out = np.maximum(np.zeros(shape=x.shape), x)
    cache = x
    return out, cache
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (out, x_norm, beta, gamma,
                 x_minus_mean, ivar, sqrtvar, var, eps)
